	...
	<table cellpadding="0" cellspacing="0" width="100%" class="sTable mTable">
		<thead>
			<tr>
				...
				<td>
					<?php echo $this->Paginator->sort("$ModelName.nama_jabatan",'Nama Jabatan');?>
				</td>
				<td>
		...
		<tbody>
			<?php foreach($data as $data): ?>
				...
				<tr <?php echo $class?>>
					...
					<td><?php echo $data[$ModelName]['nama_jabatan'] ?></td>
					...
					<td align="middle">
						<?php if($profile['Karyawan']['role'] == 1): ?>
							...
							<?php echo $this->Html->link("View", array('action' => 'View', $data[$ModelName]['id'])); ?>
						<?php else: ?>
							<?php echo $this->Html->link("View", array('action' => 'View', $data[$ModelName]['id'])); ?>
						<?php endif; ?>						
					</td>
